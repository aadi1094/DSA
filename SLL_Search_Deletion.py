...
# Traverse the linked list
    ...
    def searchSLL(self,nodeValue):
        if self.head is None:  # Check if the linked list is empty
            ...
            node = self.head  # Start from the head of the linked list
            while node is not None:  # Traverse the linked list until the end is reached
                if node.value == nodeValue:  # Check if the value of the current node matches the search value
                    return node.value  # Return the search value if found
                node = node.next  # Move to the next node
    ...
    def deleteNode(self,location):
        if self.head is None:  # Check if the linked list is empty
            print("The Singly Linked List does not exist")# Print a message if the linked list does not exist
        else:
            if location == 0:# Check if the node to be deleted is the head
                if self.head == self.tail:# Check if the linked list has only one node
                    self.head = None # Set the head to None
                    self.tail = None # Set the tail to None
                else:
                    self.head = self.head.next # Set the next node as the new head
            elif location == 1: # Check if the node to be deleted is the tail
                if self.head == self.tail:# Check if the linked list has only one node
                    self.head = None
                    self.tail = None
                else:
                    node = self.head # Start from the head of the linked list
                    while node is not None: # Traverse the linked list until the end is reached
                        if node.next == self.tail: # Check if the next node is the tail
                            break
                        node = node.next # Move to the next node
                    node.next = None # Set the next pointer of the current node to None
                    self.tail = node # Set the current node as the new tail
            else:
                tempNode = self.head # Start from the head of the linked list
                index = 0 # Initialize the index counter
                while index < location - 1 and tempNode is not None: # Traverse the linked list until the desired location is reached or the end is reached
                    ...
                    index += 1
                nextNode = tempNode.next # Store the next node after the desired location
                tempNode.next = nextNode.next    # Set the next pointer of the current node to the node after the next node   
